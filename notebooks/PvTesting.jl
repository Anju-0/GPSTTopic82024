...
# ╔═╡ 7ea56ef1-5c02-40d3-b4c4-4ea275146773
md"""
## Dependencies
"""
...
begin
	...
	NUMBER_OF_PV_PER_LOAD = 2
end

# ╔═╡ ffae988a-f742-4a4f-8443-db56a8a9522b

begin
	pvBusses, passiveBusses = assignPvLoadBusses(TEST_NETWORK, NUMBER_OF_PV_PER_LOAD)
	cd("../../")
	println("Assigned OpenDSS PV Busses: ($(length(pvBusses)))")
	for i in 1:convert(Int, ceil(length(pvBusses)/5))
		startWindow = (i - 1) * 5 + 1
		endWindow = i * 5
		if endWindow > length(pvBusses)
			println(pvBusses[startWindow:end])
		else
			println(pvBusses[startWindow:endWindow])
		end
	end
	println("Assigned OpenDSS Passive Busses: ($(length(passiveBusses)))")
	for i in 1:convert(Int, ceil(length(passiveBusses)/5))
		startWindow = (i - 1) * 5 + 1
		endWindow = i * 5
		if endWindow > length(passiveBusses)
			println(passiveBusses[startWindow:end])
		else
			println(passiveBusses[startWindow:endWindow])
		end
	end
end
...
manifest_format = "2.0"
project_hash = "d97bbbd861755ccf4fd0f5884f7ad70766292db9"
