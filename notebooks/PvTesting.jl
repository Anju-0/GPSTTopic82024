...
# ╔═╡ 8e1c262c-9d6f-11ef-22bc-13624c24fa50
# ╠═╡ show_logs = false
begin
	using Pkg
	Pkg.compat("PowerModelsDistribution", "0.15.2")
	Pkg.update()
	...
	include("./../script/PvValidation.jl")

	include("./PerPhasePlot.jl")
end

# ╔═╡ 68627277-3de2-460e-9bb8-3a4f9ebafb75
md"""
# Testing DOE Compliance with Voltage Limits
"""

# ╔═╡ 64f042ca-47ad-4706-b069-0ad64882c529
begin
	TEST_NETWORK = "./resources/ENWLNW1F1/Master.dss"
end

# ╔═╡ fe398098-e505-4dc4-b338-ba78f3d3648c
md""" ## Number of Loads per PV"""

# ╔═╡ abc712ec-dab5-4b9e-944e-afe6a9c00e94
md""" #### 2 $(@bind NUMBER_OF_LOAD_PER_PV Slider(2:1:4)) 4 """

# ╔═╡ ffae988a-f742-4a4f-8443-db56a8a9522b

begin
	pvBusses, passiveBusses = assignPvLoadBusses(TEST_NETWORK, NUMBER_OF_LOAD_PER_PV)
	cd("../../")
	...
	for i in 1:convert(Int, ceil(length(pvBusses)/5))
		...
		if endWindow > length(pvBusses)
		    global outputStr *= "$(pvBusses[startWindow:end]) \n"
		else
			global outputStr *= "$(pvBusses[startWindow:endWindow]) \n"
		end
	end
	...
	for i in 1:convert(Int, ceil(length(passiveBusses)/5))
		...
		if endWindow > length(passiveBusses)
			global outputStr *= "$(passiveBusses[startWindow:end])\n"
		else
			global outputStr *= "$(passiveBusses[startWindow:endWindow])\n"
		end
	end
    println(outputStr)
end
...
# ╠═╡ show_logs = false
pvBusExportLims = runDoeSimulation(TEST_NETWORK, pvBusses)

# ╔═╡ e4d44a82-293c-46df-8c07-fec5dd7fa4e2
md"""## Without DOE Lims"""

# ╔═╡ 2be7211d-e925-476b-863e-c50d21080121
md"""### Maximum Irradiance for PV"""

# ╔═╡ c218b716-0b6a-47a5-9b6a-4723ffeb2a0c
md"""#### 1kW/m^2 $(@bind irradMax Slider(1.0:0.01:2.0, default=0.8)) 2 kW/m^2"""
...
md"""### Voltages with DOE Export Limits Applied"""
...
begin
	...
	correctedLoadBusVoltages = collectAllLoadBusVMagAngle()
	checkVoltageLimits(correctedLoadBusVoltages, voltageBase, vLimsPu)
	
	correctedLineCurrents = collectAllLineCurrentMagAngle()
	checkCurrentLimits(correctedLineCurrents) 
	p2, _, _ = plot_3ph_load(ymax=GLOBAL_MAX, ymin=GLOBAL_MIN)
	p2
end
...
manifest_format = "2.0"
project_hash = "5a0f5725b258fec8679c8864b23d649b9d59474a"
...
git-tree-sha1 = "76315a7100e9e901d959c25b6ab6d4a37e799132"
